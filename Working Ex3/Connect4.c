...
int availCol(void* state, int col);
...
void printBoard(int *** board){
	...
	for (i=5; i>=0; i--){
		...
		for (j=0;j<7;j++){
			switch ((*board)[i][j]){
			/// 1 represents O's, -1 represents X's, and 0 represents blank spaces
				case 1: printf(" O");
				break;
				case -1: printf(" X");
				break;
...
int evalFunc(void* state){
	...
	int score = 0;
	int sum_four;
	int values[] = { -5, -2, -1, 0, 1, 2, 5 };
	int i=0,j=0, k=0;
	int x = 0;
	for (i = 0; i < 6; ++i){ //rows - left to right
		for (j = 0; j < 4; ++j){
			x = 0;
			sum_four = 0;
			for (k = 0; k < 4; ++k){
				sum_four += (*board)[i][j + k];
			}
			if (sum_four == 4)
				return 100000;
			if (sum_four == -4)
	...
	for (i = 0; i < 3; ++i){ //columns - up to down
		for (j = 0; j < 7; ++j){
			sum_four = 0;
			for (k = 0; k < 4; ++k){
				sum_four += (*board)[i + k][j];
			}
			if (sum_four == 4)
				return 100000;
			if (sum_four == -4)
				return -100000;
			score += values[sum_four + 3];
		}
	}
	for (i = 0; i < 3; ++i){
		for (j = 0; j < 4; ++j){
